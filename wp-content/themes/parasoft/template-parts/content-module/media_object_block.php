...
$parallaxdiv = '';
$bg = '';
$sectionclass[] = 'media-object-block';
...
<section <?php echo !empty($custom_id) ? ' id="' . $custom_id . '" ' : ''; ?> class="<?php echo $sec_class; ?>" <?php echo $parallaxdiv; ?> style="<?php echo $bg . $othercss; ?>">
    ...
    <?php
     $media_start_position = get_sub_field('media_start_position');
	 $content_group = get_sub_field('content');
	 $media_group = get_sub_field('media_group');
	 $choose_circle = $media_group['choose_circle'];
	
	 if($choose_circle == 1){
		 $circleclass = 'round-circle';
	 }else{
		$circleclass = 'no-circle';
	 }
    ?>
  <div class="container">
	  <?php if($media_start_position == 'left'){ ?>
		<div class="row animation-box">
		   <div class="col-xl-5 col-lg-5 col-md-6 col-sm-12 animation bottom-up">
				<div class="media-object-block-bg">
					<div class="image-block <?php echo $circleclass; ?>">
						<img src="<?php echo esc_url( $media_group['image']['url'] ); ?>" alt="<?php echo esc_attr( $media_group['image']['alt'] ); ?>" />
						<a href="<?php echo esc_url( $media_group['image']['url'] ); ?>" class="media-object-bg-popup"></a>
					</div>
				</div>
			</div>
			<div class="col-xl-7 col-lg-7 col-md-6 col-sm-12 animation bottom-up">
				<div class="media-object-block-cont">
					<?php echo!empty($content_group) ? '<h2' . $font_style_head . '>' . $content_group['heading'] . '</h2>' : ''; ?>
