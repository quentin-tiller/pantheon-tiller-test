		...
		<div class="row">
			<div class="<?php echo $widthclass; ?> col-md-12 col-sm-12">
				<div class="open-content-cont text-<?php echo $alignwidth; ?>">
                    ...
                    <div class="tech-spec-block-buttons">
                        <button class="small" id="tech-spec-expand">Expand All</button>
                        <button class="small" id="tech-spec-collapse">Collapse All</button>
                    </div>

                    <?php $groups = get_sub_field('groups'); if($groups){ ?>
                        <ul class="tech-spec-block-accordion">
                            <?php foreach($groups as $i => $group){ ?>
                                <li>
                                    <input type="checkbox" id="tech-spec-<?=$i?>">
                                    <label for="tech-spec-<?=$i?>"><?=$group['label']?></label>
                                    <div class="tech-spec-content">
                                        <?php if($group['content_2']){ ?>
                                            <div class="tech-spec-col"><?=$group['content']?></div>
                                            <div class="tech-spec-col"><?=$group['content_2']?></div>
                                        <?php }else{ ?>
                                            <div class="tech-spec-col full-width"><?=$group['content']?></div>
                                        <?php } ?>
                                    </div>
                                </li>
                            <?php } ?>
